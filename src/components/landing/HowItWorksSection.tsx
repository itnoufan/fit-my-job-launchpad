      ...
      <div className="grid md:grid-cols-3 gap-6 max-w-3xl mx-auto">
        {steps.map(({ num, title, desc, emoji }, i) => (
          <ScrollReveal key={num} delay={i * 0.15}>
            <div className="bg-card rounded-2xl p-6 text-center space-y-4 card-elevated card-hover">
              <div className="text-4xl">{emoji}</div>
              <div className="mx-auto w-10 h-10 rounded-xl bg-gradient-cta flex items-center justify-center text-sm font-extrabold text-primary-foreground">
                {num}
              </div>
